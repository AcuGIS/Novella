...
$app->post('/metadata', function (Request $request, Response $response) {
    try {
        ...
        $requiredFields = ['title', 'abstract', 'west_longitude', 'east_longitude', 'south_latitude', 'north_latitude', 'is_qgis_project'];
        ...
        if($parsedBody['is_qgis_project'] == 1){
            $proto = empty($_SERVER['HTTPS']) ? 'http' : 'https';
            
            $db = $this->get(PDO::class);
            $stmt = $db->prepare("UPDATE metadata_records SET wms_url=:wms_url WHERE id=:metadata_id");
            $stmt->execute([
                'wms_url' => $proto.'://'.$_SERVER['HTTP_HOST'].'/qgis/'.$metadataResult['id'],
                'metadata_id' => $metadataResult['id'],
            ]);

            $stmt = $db->prepare("UPDATE inspire_metadata SET spatial_data_service_url=:spatial_url WHERE metadata_id=:metadata_id");
            $stmt->execute([
                'spatial_url' => $proto.'://'.$_SERVER['HTTP_HOST'].'/qgis/'.$metadataResult['id'].'?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0',
                'metadata_id' => $metadataResult['id'],
            ]);
        }
        ...
        if (!empty($uploadedFiles)) {
            ...
            if (isset($uploadedFiles['gis_files'])) {
                foreach ($uploadedFiles['gis_files'] as $file) {
                    ...
                    // if its a QGIS project
                    if($parsedBody['is_qgis_project'] == 1){
                        $qgis_proj_dir = '/var/www/data/qgis/'.$metadataResult['id'];
                        mkdir($qgis_proj_dir);
                        
                        $user_qgis_dir = '/var/www/data/qgis/'.$_SESSION['user_id'];
                        # remove user temp qgis dir
                        if(is_dir($user_qgis_dir)){
                            removeDirectory($user_qgis_dir);
                        }
                        ...
                        if(str_ends_with($filepath, '.zip')){
                            ...
                            $zip->extractTo($qgis_proj_dir);
                            $zip->close();
                            
                            // rename .qgs file
                            $qgis_file = find_file_by_type($qgis_proj_dir, 'qgs');    
                            if ($qgis_file == null) {
                                throw new Exception('No .qgs file found in the ZIP archive');
                            }
                            rename($qgis_file, dirname($qgis_file).'/project.qgs');
                            
                        }else{
                            copy($filepath, $qgis_proj_dir.'/'.$file->getClientFilename());
                        }
                    }
        ...
        if (!empty($uploadedFiles)) {
            ...
            if (isset($uploadedFiles['gis_files'])) {
                foreach ($uploadedFiles['gis_files'] as $file) {
                    ...
                    // if its a QGIS project
                    if($data['is_qgis_project'] == 1){
                        $qgis_proj_dir = '/var/www/data/qgis/'.$metadataId;
                        
                        removeDirectory($qgis_proj_dir);
                        mkdir($qgis_proj_dir);
                        ...
                        if(str_ends_with($filepath, '.zip')){
                            ...
                            // rename .qgs file
                            $found = false;
                            $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($qgis_proj_dir, RecursiveDirectoryIterator::SKIP_DOTS));                            
                            foreach ($iterator as $it) {
                                if (strtolower($it->getExtension()) === 'qgs') {
                                    $found = true;
                                    rename($it->getPathname(), dirname($it->getPathname()).'/project.qgs');
                                    break;
                                }
                            }
        ...
        try {
            ...
            foreach ($gisFiles as $file) {
                ...
                try {
                    ...
                    if(str_ends_with($fileToProcess, '.qgs')){
                        $xml = simplexml_load_file($fileToProcess);
                        list($DefaultViewExtent) = $xml->xpath('/qgis/ProjectViewSettings/DefaultViewExtent');
                        
                        $metadata['west_longitude'] = (string) $DefaultViewExtent['xmin'];
                        $metadata['south_latitude'] = (string) $DefaultViewExtent['ymin'];
                        $metadata['east_longitude'] = (string) $DefaultViewExtent['xmax'];
                        $metadata['north_latitude'] = (string) $DefaultViewExtent['ymax'];
                        
                        list($ellipsoidacronym) = $xml->xpath('/qgis/ProjectViewSettings/DefaultViewExtent/spatialrefsys/ellipsoidacronym');
                        $metadata['coordinate_system'] = (string) $ellipsoidacronym;
                        error_log("Found coordinate system from QGIS file: " . $metadata['coordinate_system']);
                        ...
                        if (!mkdir($user_qgis_dir)) {
                            throw new Exception('Failed to create extraction directory');
                        }
                        
                        if($extractDir){
                            // extract zipped qgis project into user temp qgis dir
                            $zip = new ZipArchive();
                            $zipResult = $zip->open($tempPath);
                            $zip->extractTo($user_qgis_dir);
                            $zip->close();
                        }
                        
                        copy($fileToProcess, $user_qgis_dir.'/project.qgs');
...
EOF',
            ...
            $response->getBody()->write(json_encode([
                ...
                'is_qgis_project' => $is_qgis_project,
                'wms_url' => ($is_qgis_project == 1) ? $proto.'://'.$_SERVER['HTTP_HOST'].'/qgis/'.$_SESSION['user_id'] : null
            ]));
            return $response->withHeader('Content-Type', 'application/json');
